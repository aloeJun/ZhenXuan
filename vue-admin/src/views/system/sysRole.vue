...
import { GetSysRoleListByPage , CreateSysRole , UpdateSysRole , DeleteSysRoleById ,GetSysRoleMenuIds , DoAssignMenuIdToSysRole} from '@/api/sysRole';
...
const submit = async () => {
  if (!sysRole.value.id) {//没有id，添加操作
    const { code } = await CreateSysRole(sysRole.value)
    if (code === 200) {
      //关闭弹框
