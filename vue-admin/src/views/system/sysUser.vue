...
import { GetSysUserListByPage, CreateSysUser, UpdateSysUser, DeleteSysUserById, DoAssignRoleToUser } from '@/api/sysUser';
...
const submit = async ()=>{
    if(!sysUser.value.id) {//没有id进行添加操作
        const {code} = await CreateSysUser(sysUser.value)
        if(code === 200) {
